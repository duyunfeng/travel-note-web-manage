<template>
  <div>
    <el-dialog v-model="uploadDialog" title="批量导入" width="500" center>
      <el-upload
        class="upload-demo"
        drag
        action="http://121.37.10.32:3000/api/upload/mulResource"
        :headers="{
    ...
    <el-dialog v-model="opTableDialog" title="查看日志" width="600" center>
      <div class="table">
  ...
  if (val.id) {
    getLog({ id: val.id, name: props.name }).then((res: any) => {
      tableData.value = res.data.map((item: any) => {
        item.op = item.op.slice(1, item.op.length - 1);
        return item;
      });
